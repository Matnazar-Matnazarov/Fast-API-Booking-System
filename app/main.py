...
async def say_hello(name: str):
    try:
        ...
        return {"message": f"Hello {processed_name}", "status": "success"}
    except ValueError as ve:
        return JSONResponse(
            status_code=400, content={"message": str(ve), "status": "error"}
        )
...
async def clear_cache():
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"message": "Error clearing cache", "error": str(e)},
        )
